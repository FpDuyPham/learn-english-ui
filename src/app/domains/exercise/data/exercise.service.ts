    ...
    return from(
      db.transaction('rw', db.exercises, db.sentences, async () => {
        ...
        const sentenceIds = await db.sentences.bulkAdd(
          exercise.sentences.map((sentence: any) => ({
            ...sentence,
            exerciseId,
          }))
        );
  ...
  updateExercise(exercise: Exercise): Observable<void> {
    return from(
      db.transaction('rw', db.exercises, db.sentences, async () => {
        ...
        const sentencesToUpdate = exercise.sentences.filter((s: any) =>
          currentSentences.some((cs: Sentence) => cs.id === s.id)
        );
        const sentencesToDelete = currentSentences.filter(
          (cs: Sentence) => !exercise.sentences.some((s: any) => s.id === cs.id)
        );

        // Add new sentences
        await db.sentences.bulkAdd(
          sentencesToAdd.map((sentence: any) => ({
            ...sentence,
        ...
        await Promise.all(
          sentencesToUpdate.map((sentence: any) =>

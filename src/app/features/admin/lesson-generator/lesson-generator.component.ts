...
import { WhisperService, WhisperOutput, WhisperProgress } from '../../../core/services/whisper.service';
...
export class LessonGeneratorComponent implements OnInit, OnDestroy {
    ...
    nextStep() {
        if (this.activeIndex < 2) {
            ...
            if (this.activeIndex === 1) {
                // Initialize Wavesurfer when entering Step 2 (Index 1)
                setTimeout(() => {
    ...
    onActiveIndexChange(event: number) {
        // Only allow going to previous steps or next if data is ready
        if (event < this.activeIndex || (event === 1 && this.selectedFile && this.scriptText) || (event === 2 && this.segments.length > 0)) {
            this.activeIndex = event;
            // Resize wavesurfer when it becomes visible
            setTimeout(() => {
                if (this.wavesurfer) {
                    this.wavesurfer.setTime(0); // Redraw
                }
            }, 100);
        }
    }
    ...
    async initWavesurfer(file: File) {
        ...
        // Update segment on region update
    ...
    generateLesson() {
        ...
        // Use type assertion if needed, or ensure service accepts File
